...
object CassandraAnalyticsClient {
  ...
  def apply(config: Config, session: Session)(implicit ec: ExecutionContext): Future[CassandraAnalyticsClient] = {
    ...
    for {
      ...
      """.stripMargin).asScala
      journalSequenceNumbersView <- JournalSequenceNumbersView(keyspace, session)
      zonesView                  <- ZonesView(keyspace, session)
      balancesView               <- BalancesView(keyspace, session)
      clientsView                <- ClientsView(keyspace, session)
  ...
  object JournalSequenceNumbersView {
    def apply(keyspace: String, session: Session)(implicit ec: ExecutionContext): Future[JournalSequenceNumbersView] =
      for {
        _ <- session.executeAsync(s"""
                                     ...
                                     |);
      """.stripMargin).asScala
      } yield new JournalSequenceNumbersView(keyspace, session)
  }

  class JournalSequenceNumbersView private (keyspace: String, session: Session) {

    private[this] val retrieveStatement = session
      .prepareAsync(s"""
    ...
    private[this] val updateStatement = session
      .prepareAsync(s"""
  ...
  object ZonesView {
    ...
    object MembersView {
      def apply(keyspace: String, session: Session)(implicit ec: ExecutionContext): Future[MembersView] =
        for {
          _ <- session.executeAsync(s"""
      ...
      """.stripMargin).asScala
          _ <- session.executeAsync(s"""
                                       ...
                                       |);
      """.stripMargin).asScala
        } yield new MembersView(keyspace, session)
    }

    class MembersView private (keyspace: String, session: Session) {
      ...
      private[this] val createStatement = session
        .prepareAsync(s"""
      ...
      private[this] val addUpdate = session
        .prepareAsync(s"""
    ...
    object AccountsView {
      def apply(keyspace: String, session: Session)(implicit ec: ExecutionContext): Future[AccountsView] =
        for {
          _ <- session.executeAsync(s"""
      ...
      """.stripMargin).asScala
          _ <- session.executeAsync(s"""
                                       ...
                                       |);
      """.stripMargin).asScala
        } yield new AccountsView(keyspace, session)
    }

    class AccountsView private (keyspace: String, session: Session) {
      ...
      private[this] val updatesStatement = session
        .prepareAsync(s"""
      ...
      private[this] val addUpdateStatement = session
        .prepareAsync(s"""
    ...
    object TransactionsView {
      def apply(keyspace: String, session: Session)(implicit ec: ExecutionContext): Future[TransactionsView] =
        for {
          _ <- session.executeAsync(s"""
                                       ...
                                       |);
      """.stripMargin).asScala
        } yield new TransactionsView(keyspace, session)
    }

    class TransactionsView private (keyspace: String, session: Session) {
      ...
      def update(zone: Zone, transactions: Iterable[Transaction])(implicit ec: ExecutionContext): Future[Unit] =
        for {
          statement <- updatesStatement
          _ <- Future.traverse(transactions)(
            transaction =>
              session
      ...
      private[this] val addStatement = session
        .prepareAsync(s"""
    ...
    def apply(keyspace: String, session: Session)(implicit ec: ExecutionContext): Future[ZonesView] =
      for {
        _ <- session.executeAsync(s"""
      ...
      """.stripMargin)
          .asScala
        membersView      <- MembersView(keyspace, session)
        accountsView     <- AccountsView(keyspace, session)
        transactionsView <- TransactionsView(keyspace, session)
      } yield new ZonesView(keyspace, session, membersView, accountsView, transactionsView)
  ...
  class ZonesView private (keyspace: String,
                           ...
                           val transactionsView: TransactionsView) {
    ...
    private[this] val changeNameStatement = session
      .prepareAsync(s"""
    ...
    private[this] val addNameChangeStatement = session
      .prepareAsync(s"""
    ...
    private[this] def addNameChange(zoneId: ZoneId, changed: Long, name: Option[String])(
        implicit ec: ExecutionContext): Future[Unit] =
      for {
        statement <- addNameChangeStatement
        _ <- session
          .executeAsync(
            statement.bind(
              zoneId.id,
    ...
    private[this] val updateModifiedStatement = session
      .prepareAsync(s"""
                       |UPDATE $keyspace.zones_by_id
                       |  SET modified = ?
                       |  WHERE id = ? AND bucket = ?
    ...
    def updateModified(zoneId: ZoneId, modified: Long)(implicit ec: ExecutionContext): Future[Unit] =
      for {
        statement <- updateModifiedStatement
        _ <- session
          .executeAsync(
            statement.bind(
              new Date(modified),
  ...
  object BalancesView {
    def apply(keyspace: String, session: Session)(implicit ec: ExecutionContext): Future[BalancesView] =
      for {
        _ <- session.executeAsync(s"""
                                     ...
                                     |);
      """.stripMargin).asScala
      } yield new BalancesView(keyspace, session)
  }

  class BalancesView private (keyspace: String, session: Session) {

    private[this] val retrieve = session
      .prepareAsync(s"""
  ...
  object ClientsView {
    def apply(keyspace: String, session: Session)(implicit ec: ExecutionContext): Future[ClientsView] =
      for {
      ...
      """.stripMargin).asScala
      } yield new ClientsView(keyspace, session)
  }

  class ClientsView private (keyspace: String, session: Session) {
    ...
    private[this] val updateZoneStatement = session
      .prepareAsync(s"""
  ...
  implicit class RichListenableFuture[A](val listenableFuture: ListenableFuture[A]) extends AnyVal {

...
object LiquidityAnalytics {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val cassandraViewClient = CassandraViewClient(config, readJournal.session.underlying())
    val zoneViewShardRegion = ClusterSharding(system).start(
      typeName = ZoneViewActor.ShardName,
      entityProps = ZoneViewActor.props(readJournal, cassandraViewClient),
    ...
    system.actorOf(
      ClusterSingletonManager.props(
        singletonProps = ZoneViewStarterActor.props(readJournal, zoneViewShardRegion),

...
object ProtoBindings {
  ...
  implicit final val ZoneGrpcErrorNotificationProtoBinding
    ...
    ProtoBinding.instance(
      (s, _) =>
        proto.grpc.protocol.Errors(s.errors.toList.map(e =>
          proto.grpc.protocol.Errors.Error(e.code, e.description))),
      (p, _) =>
        Errors(
  ...
  implicit final val ZoneGrpcNotificationProtoBinding

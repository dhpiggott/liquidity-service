...
sealed abstract class LowPriorityImplicits extends LowerPriorityImplicits {
  ...
  ): ProtoBinding[SH :: STRepr, PH :: PTRepr, C] =
    new ProtoBinding[SH :: STRepr, PH :: PTRepr, C] {
  ...
  implicit def coproductProtoBinding[SL,
    ...
    new ProtoBinding[SL :+: SRRepr, PL :+: PRRepr, C] {
      override def asProto(s: SL :+: SRRepr)(implicit c: C): PL :+: PRRepr =
        s match {
          case Inl(head) => Inl(lBinding.value.asProto(head))
          case Inr(tail) => Inr(rBinding.value.asProto(tail))
        }
      override def asScala(p: PL :+: PRRepr)(implicit c: C): SL :+: SRRepr =
        p match {

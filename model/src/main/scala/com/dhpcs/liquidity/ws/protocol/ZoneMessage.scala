...
object ZoneResponse {
  ...
  object Error {
    ...
    def authorizationNotAnOAuth2BearerToken(other: HttpCredentials) =
      ZoneResponse.Error(
        code = 2,
        description = s"Expected OAuth2BearerToken but found $other."
      )
    ...
    val tokenSubjectMustBeAnRsaPublicKey =
      ZoneResponse.Error(
        code = 6,
        description = "Token subject must be an RSA public key."
      )
    ...
    val noPublicKeys =
      ZoneResponse.Error(
        code = 9,
        description = "At least one public key must be specified."
      )
    ...
    val noMemberIds =
      ZoneResponse.Error(
        code = 12,
        description = "At least one member ID must be specified."
      )
    ...
    val memberKeyMismatch =
      ZoneResponse.Error(
        code = 15,
        description = "Client public key must match that of the member."
      )
    ...
    val accountOwnerKeyMismatch =
      ZoneResponse.Error(
        code = 17,
        description = "Client public key must match that of an owner."
      )
...
final case class ZoneStateNotification(
    zone: Option[Zone],
    connectedClients: Map[String, PublicKey]
) extends ZoneNotification
final case class ClientJoinedNotification(
    connectionId: String,
    publicKey: PublicKey
) extends ZoneNotification
final case class ClientQuitNotification(
    connectionId: String,
    publicKey: PublicKey
) extends ZoneNotification
...
final case class AccountCreatedNotification(account: Account)
    extends ZoneNotification
final case class AccountUpdatedNotification(
    actingAs: MemberId,
    account: Account
) extends ZoneNotification

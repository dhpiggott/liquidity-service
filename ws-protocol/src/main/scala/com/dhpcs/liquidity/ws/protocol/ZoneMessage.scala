...
object ZoneResponse {
  ...
  object Error {
    ...
    val noPublicKeys =
      ZoneResponse.Error(code = 1, description = "At least one public key must be specified.")
    ...
    val invalidPublicKeyLength =
      ZoneResponse.Error(code = 4, description = s"Public key length must be ${ZoneCommand.RequiredKeySize} bits.")
    val noMemberIds =
      ZoneResponse.Error(code = 5, description = "At least one member ID must be specified.")
    def memberDoesNotExist(memberId: MemberId) =
      ZoneResponse.Error(code = 6, description = s"Member ${memberId.id} must exist.")
    val memberDoesNotExist =
      ZoneResponse.Error(code = 7, description = "Member must exist.")
    val memberKeyMismatch =
      ZoneResponse.Error(code = 8, description = "Client public key must match that of the member.")
    val accountDoesNotExist =
      ZoneResponse.Error(code = 9, description = "Account must exist.")
    val accountOwnerKeyMismatch =
      ZoneResponse.Error(code = 10, description = "Client public key must match that of an owner.")
    val accountOwnerMismatch =
      ZoneResponse.Error(code = 11, description = "Member must be an account owner.")
    ...
    val destinationAccountDoesNotExist =
      ZoneResponse.Error(code = 13, description = "Destination account must exist.")
    val reflexiveTransaction =
      ZoneResponse.Error(code = 14, description = "Destination account must not also be the source account.")
    val negativeTransactionValue =
      ZoneResponse.Error(code = 15, description = "Transaction value must be positive.")
    val insufficientBalance =
      ZoneResponse.Error(code = 16,
                         description =

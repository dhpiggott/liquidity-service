...
import com.dhpcs.liquidity.client.LegacyClientKeyStore._
...
object LegacyClientKeyStore {
  ...
  def apply(filesDir: File): LegacyClientKeyStore = {
    ...
    new LegacyClientKeyStore(keyStore)
  }
...
class LegacyClientKeyStore private (keyStore: KeyStore) {
  ...
  val publicKey: RSAPublicKey = keyStore.getCertificate(EntryAlias).getPublicKey.asInstanceOf[RSAPublicKey]

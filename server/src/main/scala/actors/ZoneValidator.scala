...
object ZoneValidator {
  ...
  private object PassivationCountdown {
    def props: Props = Props(new PassivationCountdown)
...
class ZoneValidator extends PersistentActor with ActorLogging with AtLeastOnceDelivery {
  ...
  private[this] val passivationActor = context.actorOf(PassivationCountdown.props)

  private[this] val zoneId = ZoneId(UUID.fromString(self.path.name))

  private[this] var state = State()

  private[this] var nextExpectedCommandSequenceNumbers = Map.empty[ActorRef, Long].withDefaultValue(0L)
  private[this] var messageSequenceNumbers = Map.empty[ActorRef, Long].withDefaultValue(0L)
  ...
  private[this] def waitForZone: Receive = publishStatus orElse confirmMessageReceipt orElse waitForTimeout orElse {
  ...
  private[this] def withZone: Receive = publishStatus orElse confirmMessageReceipt orElse waitForTimeout orElse {
    case AuthenticatedCommandWithIds(publicKey, command, correlationId, sequenceNumber, deliveryId) =>
      passivationActor ! CommandReceivedEvent
      val nextExpectedCommandSequenceNumber = nextExpectedCommandSequenceNumbers(sender())
      if (sequenceNumber <= nextExpectedCommandSequenceNumber) {
  ...
  private[this] def confirmMessageReceipt: Receive = {
